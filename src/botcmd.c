...
static void bot_priv (int idx, char * par)
{
   ...
   if (!to[0]) 
     return; /* silently ignore notes to '@bot' this is legacy code */
   if (rfc_casecmp(tobot, botnetnick) == 0) {	/* for me! */
      if (p == from)
...
static void bot_who (int idx, char * par) {
   ...
   to = newsplit(&par);
   if (rfc_casecmp(to, botnetnick) == 0)
      to[0] = 0;		/* (for me) */
#ifndef NO_OLD_BOTNET
   if (b_numver(idx) < NEAT_BOTNET)
     chan = atoi(par);
...
static void bot_link (int idx, char * par) {
   ...
   if (rfc_casecmp(bot, botnetnick) == 0) {
...
static void bot_nlinked (int idx, char * par) {
   ...
   } else if ((in_chain(newbot)) || (rfc_casecmp(newbot, botnetnick) == 0)) {
      /* loop! */
...
static void bot_trace (int idx, char * par) {
   ...
   if (rfc_casecmp(dest, botnetnick) && ((i = nextbot(dest)) > 0))
...
static void bot_reject (int idx, char * par) {
   ...
   if (!(destbot = strchr(who, '@'))) {
	 ...
	 botnet_send_priv(idx, botnetnick, from, NULL, "%s %s (%s)",
      ...
      } else if (rfc_casecmp(dcc[i].nick, who) == 0) {
	 char s[1024];
      ...
      *destbot++ = 0;
      if (rfc_casecmp(destbot, botnetnick) == 0) {
	 /* kick someone here! */
	 int ok = 0;
	 if (remote_boots == 1) {
	    frombot = strchr(from, '@');
	 ...
	 for (i = 0; (i < dcc_total) && (!ok); i++)
	   if ((rfc_casecmp(who, dcc[i].nick) == 0) && 
...
static void bot_thisbot (int idx, char * par) {
   context;
   if (rfc_casecmp(par, dcc[idx].nick) != 0) {
      char s[1024];
...
static void bot_handshake (int idx, char * par) {
   ...
   context;
   if (u_pass_match(u,"-")) {
      noshare = 1; /* we *don't* want botnet passwords migrating */
...
static void bot_filereq (int idx, char * tobot) {
   ...
   if ((path = strchr(tobot,':'))) {
      *path++ = 0;
      ...
      if (!rfc_casecmp(tobot, botnetnick)) {	/* for me! */
	 /* process this */
...
static void bot_join (int idx, char * par) {
   ...
   if (sock == 0)
      sock = fakesock++;
   i = nextbot(bot);
   if (i != idx) { /* ok, garbage from a 1.0g (who uses that now?) OR
		    * raistlin being evil :) */
      fake_alert(idx, "direction", bot);
      return;
   }
...
static void bot_versions (int sock, char * par) {
   ...
   else if (rfc_casecmp(tobot = newsplit(&par), botnetnick)) {
