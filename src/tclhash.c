...
p_tcl_bind_list H_bot, H_time, H_nkch, H_away, H_note, H_filt;
...
static int unbind_bind_entry (p_tcl_bind_list typ,
			  char * flags,
			  char * cmd, 
			  char * proc)
{
...
static int bind_bind_entry (p_tcl_bind_list typ,
			    char * flags,
			    char * cmd, 
			    char * proc)
{
   ...
   for (hm = bind->first; hm && !f; ohm = hm,hm = hm->next) {
	 ...
	 if (match_type & BIND_STACKABLE) {
	    ...
	    if ((match_type & 3) != MATCH_MASK)
	      f = 1; /* this will suffice until we have stackable partials */
	 } else {
	    if (match_type & BIND_USE_ATTR) {
	       if (match_type & BIND_HAS_BUILTINS)
		 atrok = flagrec_ok(&tt->flags, atr);
...
void check_tcl_link (char * bot, char * via)
{
   ...
   context;
   check_tcl_bind(H_link, bot, 0, " $_link1 $_link2", MATCH_MASK | BIND_STACKABLE);
   context;
}
