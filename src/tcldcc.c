  ...
  for (arg=2; arg<argc; arg++) {
    ...
    for (j=0; j<strlen(argv[arg]); j++) {
      if (argv[arg][j]=='+') pls=1;
       ...
       char s[2];
       s[0]=argv[arg][j]; s[1]=0;
       if (pls==1) dcc[i].u.chat->strip_flags|=stripmodes(s);
       else dcc[i].u.chat->strip_flags&=~stripmodes(s);
      }
    }
  }
...
int tcl_unlink STDVAR
{
  ...
  i=nextbot(bot); if (i<0) x=0;
    ...
    x=1;
    if (strcasecmp(bot,dcc[i].nick)==0) x=botunlink(-2,bot,argv[2]);
    else
...
{
  int i,j,idx=(-1),port; char s[10];
  BADARGS(3,4," port type ?mask/proc?");
  port=atoi(argv[1]);
  for (i=0; i<dcc_total; i++)
    if ((dcc[i].type==DCC_TELNET) && (dcc[i].port==port)) idx=i;
  if (strcasecmp(argv[2],"off")==0) {
    ...
    killsock(dcc[idx].sock); lostdcc(idx); return TCL_OK;
  }
  if (idx<0) {
    /* make new one */
    if (dcc_total>=MAXDCC) {
    ...
    if (i<0) {
      ...
      killsock(dcc[idx].sock); dcc_total--;
      return TCL_ERROR;
    }
    dcc[idx].port=port;
    dcc[idx].sock=i;
  ...
  /* bots/users/all */
  dcc[idx].nick[0]=0;
  if (strcasecmp(argv[2],"bots")==0) strcpy(dcc[idx].nick,"(bots)");
  else if (strcasecmp(argv[2],"users")==0) strcpy(dcc[idx].nick,"(users)");
  else if (strcasecmp(argv[2],"all")==0) strcpy(dcc[idx].nick,"(telnet)");
  if (!dcc[idx].nick[0]) {
