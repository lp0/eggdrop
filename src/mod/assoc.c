...
int cmd_assoc PROTO2(int,idx,char *,par)
{
   ...
   if (!par[0]) {
	 ...
	 return 0;
      ...
      if (chan<100000) tandout_but(-1,"assoc %d 0\n",chan);
      return 0;
   }
   if (strlen(par)>20) {
...
char * assoc_start PROTO((module_function *));
   
static module_function assoc_table[] = {
   (module_function)assoc_start,
   (module_function)assoc_close,
   (module_function)assoc_expmem,
   (module_function)assoc_report,
};

char * assoc_start PROTO1(module_function *,egg_func_table) {
