...
static int msg_hello (char * nick, char * h, struct userrec * u, char * p)
{
   ...
   if (get_user_by_handle(userlist,nick)) {
      struct flag_record fr = { FR_GLOBAL, 0, 0, 0, 0, 0 };
      fr.global = atr;
      dprintf(DP_SERVER, IRC_BADHOST1, IRC_BADHOST1_ARGS);
      dprintf(DP_SERVER, IRC_BADHOST2, IRC_BADHOST2_ARGS);
      return 1;
   }
   ...
   dprintf(DP_SERVER, IRC_SALUT1, IRC_SALUT1_ARGS);
   dprintf(DP_SERVER, IRC_SALUT2, IRC_SALUT2_ARGS);
   if (common) {
      dprintf(DP_SERVER, "NOTICE %s :%s", nick, IRC_SALUT2A);
      dprintf(DP_SERVER, "NOTICE %s :%s", nick, IRC_SALUT2B);
   }
   if (make_userfile) {
      fr.global = sanity_check(default_flags | USER_OWNER);
      dprintf(DP_SERVER, "NOTICE %s :%s\n", nick, IRC_INITOWNER1);
      dprintf(DP_SERVER, IRC_NEWBOT1, IRC_NEWBOT1_ARGS);
      dprintf(DP_SERVER, IRC_NEWBOT2, IRC_NEWBOT2_ARGS);
      ...
      dprintf(DP_SERVER, IRC_INTRO1, IRC_INTRO1_ARGS);
   }
   if (notify_new[0]) {
...
static int msg_ident (char * nick, char * host, struct userrec * u, char * par)
{
   ...
   if (!u2) {
	 ...
	 if (!quiet_reject) 
	   dprintf(DP_SERVER, IRC_MISIDENT, IRC_MISIDENT_ARGS2);
	 return 1;
      } else {
...
static int msg_info (char * nick, char * host, struct userrec * u, char * par)
{
   ...
   if (par[0]) {
	...
	locked = 1;
	 ...
	 if (chname) {
	    ...
	    putlog(LOG_CMDS, "*", "(%s!%s) !%s! INFO %s NONE", nick, host, u->handle,
		   chname);
	 } else {
   ...
   if (p && p[0]) {
      ...
      dprintf(DP_SERVER, "NOTICE %s :%s /msg %s info <pass>%s%s none\n",
