...
/* new server port? */
static int newserverport;
/* new server password? */
static char newserverpass[121];
/* trying to connect to a server right now? */
static time_t trying_server;
/* how lagged (in seconds) is the server? */
static int server_lag;
/* possible alternate nickname to use */
static char altnick[NICKLEN];
/* temporary thing for nick changes */
static char newbotname[NICKLEN];
...
static char botuserhost[121];
/* keep trying to regain my intended nickname? */
static int keepnick;
/* Check for a stoned server? */
static int check_stoned;
/* Disconnect from server if ERROR messages received? */
static int serverror_quit;
/* Quietly reject dcc chat or sends to users without access? */
static int quiet_reject;
/* set when i unidle myself, cleared when i get the response */
static int waiting_for_awake;
/* server connection time */
static time_t server_online;
/* seconds to wait before re-beginning the server list */
static time_t server_cycle_wait;
/* realname of bot */
static char botrealname [121];
/* minimum number of servers to be around */
static int min_servs;
/* server timeout for connecting */
static int server_timeout;
/* never give up when connecting to servers? */
static int never_give_up;
/* don't update server list */
static int strict_servernames;
/* old-style queue, still used by server list */
static struct server_list *serverlist;
/* cycle time till next server connect */
static int cycle_time;
/* default IRC port */
static int default_port;
/* previous nickname *before* rehash */
static char oldnick[NICKLEN];
/* trigger bindings if user is being ignored ? */
static int trigger_on_ignore;
/* answer how many stacked ctcp's ? */
static int answer_ctcp;
/* answer lowercase CTCP's (lame & non-standard ;) */
static int lowercase_ctcp;
...
static void queue_server (int which, char * buf, int len) {
   ...
   if (which == DP_MODE)
     deq_msg(); /* DP_MODE needs to be ASAP, flush if possible */
}
