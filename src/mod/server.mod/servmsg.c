...
static void check_tcl_msgm (char * cmd, char * nick,
		     char * uhost, struct userrec * u,
		     char * arg)
{
   struct flag_record fr = {FR_GLOBAL|FR_CHAN|FR_ANYWH,0,0,0,0,0};
...
static void fixfrom (char * s)
{
   ...
   if ((p = strchr(s, '!')))
	  ...
	  p = s; /* sometimes we get passed just a user@host here... */
   /* these are ludicrous. */
   if (strchr("~+-^=", *p))
      strcpy(p, p+1);
}
...
static int gotmode (char * from, char * msg)
{
   ...
   if ((ch[0] != '#') && (ch[0] != '&')) {
	 ...
	 /* umode +r? - D0H dalnet uses it to mean something differnt */
	 fixcolon(msg);
	 if ((msg[0] == '+') 
	     && strchr(msg, 'r')) {
	    int servidx = findanyidx(serv);
...
static void server_activity (int idx, char * msg, int len) {
   ...
   if (from[0])
      fixfrom(from);
   context;
   /* this has GOT to go into the raw binding table,
    * merely because this is less effecient */
