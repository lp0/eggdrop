...
void tell_verbose_status PROTO2(int,idx,int,showchan)
{
...
#ifndef NO_IRC
  struct chanset_t *chan; char *p,*q;
#endif
#if HAVE_GETRUSAGE
  struct rusage ru;
#else
  ...
  chan=chanset; while (chan!=NULL) {
    strcpy(s,chan->name);
    if (!(chan->stat & CHANACTIVE)) strcat(s," (trying)");
    else if (chan->stat & CHANPEND) strcat(s," (pending)");
    else if (!me_op(chan)) strcat(s," (want ops!)");
    strcat(s,", ");
    q=(char *)nmalloc(strlen(p)+strlen(s)+1);
    strcpy(q,p); strcat(q,s); nfree(p); p=q;
    chan=chan->next;
  }
  if (strlen(p)>10) {
    p[strlen(p)-2]=0;
  ...
  if (showchan) {
    ...
    while (chan!=NULL) {
      s[0]=0;
      if (chan->stat & CHAN_GREET) strcat(s,"greet, ");
      if (chan->stat & CHAN_OPONJOIN) strcat(s,"auto-op, ");
      if (chan->stat & CHAN_BITCH) strcat(s,"bitch, ");
      if (s[0]) s[strlen(s)-2]=0;
      if (!s[0]) strcpy(s,"lurking");
  ...
  if (!trying_server) {
    ...
    if ((server_lag) && !(waiting_for_awake)) {
      ...
      if (server_lag==(-1)) sprintf(s1," (bad pong replies)");
      strcat(s,s1);
    }
  }
  ...
  if (now>86400) {
    ...
    if ((int)(now/86400)>=2) strcat(s,"s");
    strcat(s,", "); now-=(((int)(now/86400))*86400);
  }
  hr=(time_t)((int)now/3600); now-=(hr*3600);
  min=(time_t)((int)now/60);
  sprintf(&s[strlen(s)],"%02d:%02d",(int)hr,(int)min);
  s1[0]=0;
  if (backgrd) strcpy(s1,"background");
  else {
    if (term_z) strcpy(s1,"terminal mode");
    else if (con_chan) strcpy(s1,"status mode");
    else strcpy(s1,"logdump mode");
  }
#if HAVE_GETRUSAGE
  ...
  s1[0]=0;
  if (learn_users) strcat(s1,"learn users, ");
  if (share_users) {
    if (passive) strcat(s1,"passive share, ");
    else strcat(s1,"aggressive share, ");
  }
  if (s1[0]) s1[strlen(s1)-2]=0; else strcpy(s1,"none");
...
char *getchanmode PROTO1(struct chanset_t *,chan)
{
  ...
  if (atr&CHANINV) s[i++]='i';
  if (atr&CHANPRIV) s[i++]='p';
  if (atr&CHANSEC) s[i++]='s';
  if (atr&CHANMODER) s[i++]='m';
  if (atr&CHANTOPIC) s[i++]='t';
  if (atr&CHANNOMSG) s[i++]='n';
  if (atr&CHANANON) s[i++]='a';
  if (chan->channel.key[0]) s[i++]='k';
  if (chan->channel.maxmembers>-1) s[i++]='l';
  s[i]=0;
  if (chan->channel.key[0]) sprintf(&s[strlen(s)]," %s",chan->channel.key);
...
void tell_verbose_chan_info PROTO2(int,idx, char *,chname)
{
  ...
  struct chanset_t *chan; int i,atr,chatr; time_t now; memberlist *m;
#endif
#ifdef NO_IRC
  dprintf(idx,"This bot runs without IRC interaction.\n");
#else
  if (!chname[0]) chan=findchan(dcc[idx].u.chat->con_chan);
  else chan=findchan(chname);
  if (chan==NULL) {
  ...
  if (chan->stat&CHANPEND) sprintf(s1,"Processing channel %s",chan->name);
  else if (chan->stat&CHANACTIVE) sprintf(s1,"Channel %s",chan->name);
  ...
  if (chan->stat&CHANACTIVE) {
	...
	if ((time(NULL)-(m->joined)) > 86400) {
	  strcpy(s1,&s[4]); strcpy(s,&s[8]); strcpy(&s[2],s1); s[5]=0;
	}
	else { strcpy(s,&s[11]); s[5]=0; }
      } 
      else strcpy(s," --- ");
      if (m->user==NULL) { 
	...
	get_handle_by_host(handle,s1);  /* <- automagically updates m->user */
      ...
      if (m->user==NULL) atr=0;
      else { strcpy(handle,m->user->handle); atr=m->user->flags; }
...
char *quickban PROTO2(struct chanset_t *,chan, char *,uhost)
{
  ...
  i=0; pp=strchr(uhost,'!');
  if (pp==NULL) pp=uhost; else pp++;
  while ((*pp) && (*pp!='@')) { pp++; i++; }
  if (i>9) strcpy(s,s1);
  else sprintf(s,"*!*%s",&s1[2]);  /* gotta add that extra '*' */
  add_mode(chan,'+','b',s); flush_mode(chan,QUICK);
  return s;
}
...
void tell_chanbans PROTO4(struct chanset_t *,chan, int,idx, int,k, char *,match)
{
  ...
  if (chan->stat&CHANACTIVE) {
	...
	strcpy(s,b->who); splitnick(s1,s);
	if (s1[0]) sprintf(fill,"%s (%s!%s)",b->ban,s1,s);
	else if (strcasecmp(s,"existent")==0) sprintf(fill,"%s (%s)",b->ban,s);
	else sprintf(fill,"%s (server %s)",b->ban,s);
	if (b->timer!=0) {
	  min=(now - b->timer)/60; sec=(now - b->timer)-(min*60);
...
int kill_chanban PROTO4(char *,chname,int,idx,int,k,int,which)
{
  ...
  if (k<=which) { dprintf(idx,"No such ban.\n"); return 0; }
  else return 1;
}
...
void log_chans()
{
  banlist *b; memberlist *m; struct chanset_t *chan;
  int chops,bans;
  chan=chanset; while (chan!=NULL) {
    if ((chan->stat&CHANACTIVE) && (chan->stat&CHAN_LOGSTATUS)) {
      m=chan->channel.member; chops=0;
      while (m->nick[0]) {
	if (m->flags&CHANOP) chops++;
	m=m->next;
      }
      b=chan->channel.ban; bans=0;
      while (b->ban[0]) { bans++; b=b->next; }
...
void tell_chan_info PROTO1(char *,nick)
{
  ...
  if (admin[0]) mprintf(serv,"NOTICE %s :Admin: %s\n",nick,admin);
  p=(char *)nmalloc(11); strcpy(p,"Channels: ");
  chan=chanset; while (chan!=NULL) {
    strcpy(s,chan->name);
    if (!(chan->stat & CHANACTIVE)) strcat(s," (trying)");
...
void got324 PROTO2(char *,from,char *,msg)
{
  ...
  if (chan->mode_cur!=0) chan->mode_cur=0;
  while (msg[i]!=0) {
    if (msg[i]=='i') { chan->channel.mode|=CHANINV; chan->mode_cur|=CHANINV; }
    if (msg[i]=='p') { chan->channel.mode|=CHANPRIV; chan->mode_cur|=CHANPRIV; }
    if (msg[i]=='s') { chan->channel.mode|=CHANSEC; chan->mode_cur|=CHANSEC; }
    if (msg[i]=='m') { chan->channel.mode|=CHANMODER; chan->mode_cur|=CHANMODER; }
    if (msg[i]=='t') { chan->channel.mode|=CHANTOPIC; chan->mode_cur|=CHANTOPIC; }
    if (msg[i]=='n') { chan->channel.mode|=CHANNOMSG; chan->mode_cur|=CHANNOMSG; }
    if (msg[i]=='a') { chan->channel.mode|=CHANANON; chan->mode_cur|=CHANANON; }
    if (msg[i]=='q') { chan->channel.mode|=CHANQUIET; chan->mode_cur|=CHANQUIET; }
    if (msg[i]=='k') {
      chan->mode_cur|=CHANKEY;
      p=strchr(msg,' '); p++;
      q=strchr(p,' '); if (q!=NULL) {
	*q=0; chan->channel.key=(char *)nmalloc(strlen(p)+1);
	strcpy(chan->channel.key,p); strcpy(p,q+1);
      }
      else {
	chan->channel.key=(char *)nmalloc(strlen(p)+1);
	strcpy(chan->channel.key,p); *p=0;
      }
    }
    if (msg[i]=='l') {
      chan->mode_cur|=CHANLIMIT;
      p=strchr(msg,' '); p++; q=strchr(p,' '); if (q!=NULL) {
	*q=0; chan->channel.maxmembers=atoi(p); strcpy(p,q+1);
      }
      else { chan->channel.maxmembers=atoi(p); *p=0; }
    }
    i++;
  }
}
...
void got352 PROTO2(char *,from,char *,msg)
{
  ...
  split(NULL,msg); split(s,msg);
  chan=findchan(s); if (chan==NULL) return;
  split(userhost,msg); strcat(userhost,"@");
  split(&userhost[strlen(userhost)],msg);
  split(NULL,msg); split(nick,msg);
  sprintf(s,"%s!%s",nick,userhost); fixfrom(s);
  splitnick(nick,s); strcpy(userhost,s);
  split(s,msg); m=ismember(chan,nick);
  if (m==NULL) { 
    m=newmember(chan);
    m->joined=m->split=0L; m->flags=0;
    m->last=time(NULL);
  }
  strcpy(m->nick,nick); strcpy(m->userhost,userhost); m->user=NULL;
  if (strcasecmp(nick,botname)==0) strcpy(botuserhost,userhost);
  waschanop=me_op(chan);
  if (strchr(s,'@')!=NULL) m->flags |= CHANOP;
  else m->flags &= ~CHANOP;
  if (strchr(s,'+')!=NULL) m->flags |= CHANVOICE;
  else m->flags &= ~CHANVOICE;
  if ((strcasecmp(nick,botname)==0) && (!waschanop) && (me_op(chan)))
    newly_chanop(chan);
  if ((strcasecmp(nick,botname)==0) && (any_ops(chan)) && (!me_op(chan))) {
    if (chan->need_op[0]) do_tcl("need-op",chan->need_op);
  }
  sprintf(s,"%s!%s",nick,userhost); fixfrom(s);
...
void got367 PROTO2(char *,from,char *,msg)
{
  ...
  split(NULL,msg); split(chname,msg);
  chan=findchan(chname); if (chan==NULL) return;
  nsplit(ban,msg); nsplit(who,msg);
  /* extended timestamp format? */
  if (who[0]) newban(chan,ban,who);
  else newban(chan,ban,"existent"); 
  sprintf(s,"%s!%s",botname,botuserhost);
  if ((wild_match(ban,s)) && (me_op(chan))) add_mode(chan,'-','b',ban);
...
void got368 PROTO2(char *,from,char *,msg)
{
  ...
  split(NULL,msg); split(chname,msg);
  chan=findchan(chname); if (chan==NULL) return;
  if (chan->stat&CHAN_CLEARBANS) resetbans(chan);
  else {
...
void got251 PROTO2(char *,from,char *,msg)
{
  ...
  for (i=0; i<8; i++) split(NULL,msg); /* lusers IS NOT SENT AT ALL!! */
  split(servs,msg);
  if (strncmp(msg,"servers",7)!=0) return;  /* was invalid format */
  i=atoi(servs); if (min_servs==0) return;  /* no minimum limit on servers */
  if (i<min_servs) {
...
void got471 PROTO2(char *,from,char *,msg)
{
  ...
  chan=findchan(chname); if (chan==NULL) return;
  if (chan->need_limit[0]) do_tcl("need-limit",chan->need_limit);
}
...
void got473 PROTO2(char *,from,char *,msg)
{
  ...
  chan=findchan(chname); if (chan==NULL) return;
  if (chan->need_invite[0]) do_tcl("need-invite",chan->need_invite);
}
...
void got474 PROTO2(char *,from,char *,msg)
{
  ...
  chan=findchan(chname); if (chan==NULL) return;
  if (chan->need_unban[0]) do_tcl("need-unban",chan->need_unban);
}
...
void got475 PROTO2(char *,from,char *,msg)
{
  ...
  chan=findchan(chname); if (chan==NULL) return;
  if (chan->need_key[0]) do_tcl("need-key",chan->need_key);
}
...
void gotinvite PROTO2(char *,from,char *,msg)
{
  ...
  split(NULL,msg); fixcolon(msg); splitnick(nick,from);
  ...
  strcpy(lastnick,nick); lastinv=time(NULL);
  chan=findchan(msg);
  if (chan==NULL) return;  /* some channel i don't want */
...
void gottopic PROTO2(char *,from,char *,msg)
{
  ...
  split(chname,msg); fixcolon(msg); splitnick(nick,from);
  chan=findchan(chname); if (chan==NULL) return;
...
void gotjoin PROTO2(char *,from,char *,chname)
{
  ...
  if ((!(chan->stat & CHANACTIVE)) && (strcasecmp(botname,nick)!=0) &&
    ...
    chan->stat|=CHANACTIVE; chan->stat&=~CHANPEND;
    reset_chan_info(chan);
    return;
  }
  m=ismember(chan,nick); if (m!=NULL) {
    /* already on channel?!? */
    if (m->split==0) killmember(chan,nick);
    else if (strcasecmp(m->userhost,from)!=0) killmember(chan,nick);
    else {
      check_tcl_rejn(nick,from,handle,chan->name);
      m->split=0; m->last=time(NULL);
	...
	do_embedded_mode(chan,nick,m,newmode);
      ...
      else putlog(LOG_JOIN,chname,"%s (%s) returned to %s.",nick,from,chname);
      return;
    }
  }
  m=newmember(chan);
  m->joined=time(NULL); m->split=0L;
  m->flags=0; m->last=time(NULL);
  strcpy(m->nick,nick); strcpy(m->userhost,from);
  sprintf(s,"%s!%s",nick,from); m->user=NULL;
  m->user=get_user_by_host(s);
  check_tcl_join(nick,from,handle,chname);
  set_handle_laston(chname,handle,time(NULL));
  if (newmode) do_embedded_mode(chan,nick,m,newmode);
  if (strcasecmp(nick,botname)==0) {
    /* it was me joining! */
    if (newmode) putlog(LOG_JOIN|LOG_MISC,chname,"%s joined %s (with +%s).",
			nick,chname,newmode);
    else putlog(LOG_JOIN|LOG_MISC,chname,"%s joined %s.",nick,chname);
    chan->stat|=CHANPEND;
    mprintf(serv,"MODE %s\n",chname);
    ...
    if (newmode) putlog(LOG_JOIN,chname,"%s (%s) joined %s (with +%s).",nick,
			from,chname,newmode);
    else putlog(LOG_JOIN,chname,"%s (%s) joined %s.",nick,from,chname);
    atr=get_attr_host(s);
    chatr=get_chanattr_host(s,chname);
    for (i=0; i<strlen(s); i++) if ((unsigned char)s[i]<32) {
...
void gotpart PROTO2(char *,from,char *,chname)
{
  ...
  splitnick(nick,from); fixcolon(chname);
  if (!(chan->stat & CHANACTIVE)) {
    /* whoa! */
    putlog(LOG_MISC,chname,
	   "confused bot: guess I'm on %s and didn't realize it",chname);
  ...
  else check_lonely_channel(chan);
}
...
void gotkick PROTO2(char *,from,char *,msg)
{
  ...
  int atr,atr1,chatr,chatr1; time_t now=time(NULL);
  split(chname,msg);
  chan=findchan(chname); if (chan==NULL) return;
  if (chan->stat & CHANPEND) return;  /* not ready yet */
  split(nick,msg); fixcolon(msg); splitnick(whodid,from);
  sprintf(s,"%s!%s",whodid,from);
  m=ismember(chan,whodid); if (m!=NULL) m->last=time(NULL);
  get_handle_by_host(hand,s);
  ...
  if (!(atr & (USER_FRIEND|USER_MASTER)) 
    ...
    if (now - chan->kicktime<=10) {
      chan->kicks++; if (chan->kicks>=3) {   /* masskick */
	tprintf(serv,"KICK %s %s :mass kick, go sit in a corner\n",chname,
		whodid);
	chan->kicks=0; chan->kicknick[0]=0; chan->kicktime=0L;
      }
    }
    else { chan->kicks=1; chan->kicktime=time(NULL); }
  }
  else {
    strcpy(chan->kicknick,whodid); chan->kicktime=time(NULL); chan->kicks=1;
  }
  ...
  else killmember(chan,nick);
  if (strcasecmp(nick,botname)!=0) check_lonely_channel(chan);
}
...
void gotnick PROTO2(char *,ffrom,char *,msg)
{
  ...
  strcpy(from,ffrom); fixfrom(from);
  get_handle_by_host(hand,from); strcpy(s,from);
  splitnick(nick,from); fixcolon(msg);
  chan=chanset; while (chan!=NULL) {
	...
	if (mm!=NULL) {
	  ...
	  if (mm->split)
	    putlog(LOG_JOIN,chan->name,"Possible future nick collision: %s",
		   mm->nick);
	  else putlog(LOG_MISC,chan->name,"* Bug: nick change to existing nick");
...
void gotwall PROTO2(char *,from,char *,msg)
{
  ...
  if ((p!=NULL) && (p==strrchr(from,'!'))) {
    ...
    if (r==0) putlog(LOG_WALL,"*","!%s(%s)! %s",nick,from,msg);
  } 
  else {
    r=check_tcl_wall(from,msg);
    if (r==0) putlog(LOG_WALL,"*","!%s! %s",from,msg);
  }
  return;
}
...
void gotquit PROTO2(char *,from,char *,msg)
{
  ...
  if ((p!=NULL) && (p==strrchr(msg,' '))) {
    char *z1,*z2;
    *p=0; z1=strchr(p+1,'.'); z2=strchr(msg,'.');
...
void show_all_info PROTO2(char *,chname,char *,who)
{
  ...
  m=chan->channel.member; while (m->nick[0]) {
    sprintf(s,"%s!%s",m->nick,m->userhost);
    get_handle_by_host(nick,s);
    get_handle_info(nick,info);
    atr=get_attr_handle(nick);
    if (atr & USER_BOT) info[0]=0;
    if (info[0]=='@') strcpy(info,&info[1]);
    else {
      get_handle_chaninfo(nick,chname,s);
      if (s[0]=='@') strcpy(s,&s[1]);
      if (s[0]) strcpy(info,s);
    }
    if (info[0]) hprintf(serv,"NOTICE %s :[%9s] %s\n",who,m->nick,info);
    else {
	...
	if (atr & BOT_SHARE)
      ...
      else { strcat(also,m->nick); strcat(also,", "); }
    }
    m=m->next;
  }
  if (also[0]) {
...
void user_kickban PROTO2(int,idx, char *,nick)
{
  ...
  note[0]=0;
  if (strchr(nick,' ')!=NULL) {
    split(s,nick); strncpy(note,nick,65); note[65]=0; strcpy(nick,s);
  }
...
#endif
  if (m->flags&CHANOP) add_mode(chan,'-','o',m->nick);
...
void check_for_split()
{
  ...
  static count=4; int ok=0; struct chanset_t *chan;
  if (min_servs==0) return;
  chan=chanset; while (chan!=NULL) {
    if ((chan->stat&CHANACTIVE) && (chan->channel.members==1)) ok=1;
    chan=chan->next;
  }
  if (!ok) return;
  count++;
  if (count>=5) { mprintf(serv,"LUSERS\n"); count=0; }
}
