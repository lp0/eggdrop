...
int expmem_users()
{
  ...
  while (u!=NULL) {
    ...
    ch=u->chanrec; while (ch!=NULL) {
      ...
      if (ch->info!=NULL) tot+=strlen(ch->info)+1;
      ch=ch->next;
    }
  ...
  chan=chanset; while (chan!=NULL) {
    ...
    ch=chan->bans->chanrec; while (ch!=NULL) {
      tot+=sizeof(struct chanuserrec);
      if (ch->info!=NULL) tot+=strlen(ch->info)+1;
      ch=ch->next;
    }
...
unsigned int str2chflags PROTO1(char *,s)
{
  ...
  for (i=0; i<strlen(s); i++) {
    ...
    if (s[i]==chanflag1) f|=CHANUSER_1;
    if (s[i]==chanflag2) f|=CHANUSER_2;
    if (s[i]==chanflag3) f|=CHANUSER_3;
    if (s[i]==chanflag4) f|=CHANUSER_4;
    if (s[i]==chanflag5) f|=CHANUSER_5;
    if (s[i]==chanflag6) f|=CHANUSER_6;
    if (s[i]==chanflag7) f|=CHANUSER_7;
    if (s[i]==chanflag8) f|=CHANUSER_8;
    if (s[i]==chanflag9) f|=CHANUSER_9;
    if (s[i]==chanflag0) f|=CHANUSER_0;
  }
  return f;
}
...
void chflags2str PROTO2(int,f,char *,s)
{
  ...
  if (f&CHANUSER_1) { strcat(s,"?"); s[strlen(s)-1]=chanflag1; }
  if (f&CHANUSER_2) { strcat(s,"?"); s[strlen(s)-1]=chanflag2; }
  if (f&CHANUSER_3) { strcat(s,"?"); s[strlen(s)-1]=chanflag3; }
  if (f&CHANUSER_4) { strcat(s,"?"); s[strlen(s)-1]=chanflag4; }
  if (f&CHANUSER_5) { strcat(s,"?"); s[strlen(s)-1]=chanflag5; }
  if (f&CHANUSER_6) { strcat(s,"?"); s[strlen(s)-1]=chanflag6; }
  if (f&CHANUSER_7) { strcat(s,"?"); s[strlen(s)-1]=chanflag7; }
  if (f&CHANUSER_8) { strcat(s,"?"); s[strlen(s)-1]=chanflag8; }
  if (f&CHANUSER_9) { strcat(s,"?"); s[strlen(s)-1]=chanflag9; }
  if (f&CHANUSER_0) { strcat(s,"?"); s[strlen(s)-1]=chanflag0; }
  if (s[0]==0) strcpy(s,"-");
}
...
struct chanuserrec *get_chanrec PROTO2(struct userrec *,u,char *,chname)
{
  ...
  while (ch!=NULL) {
    if (strcasecmp(ch->channel,chname)==0) return ch;
    ch=ch->next;
  }
  return NULL;
}
			...
			unsigned int,flags,time_t,laston)
{
  ...
  ch->next=u->chanrec; u->chanrec=ch;
  ch->info=NULL; ch->flags=flags; ch->laston=laston;
...
int write_user PROTO3(struct userrec *,u,FILE *,f,int,shr)
{
  ...
  if (u->lastonchan[0]) {
    if (fprintf(f,"!!        %lu %s\n",u->laston,u->lastonchan)==EOF) return 0;
  }
  if (u->xtra!=NULL) {
    char *p=u->xtra;
    while (strlen(p)>160) {
			       ...
			       char *,pass,int,flags)
{
  ...
  u->host=NULL; u->next=NULL; u->chanrec=NULL;
...
struct userrec *dup_userlist PROTO1(int,t)
{
  ...
  while (u!=NULL) {
	...
	q=q->next;
      ...
      ch=u->chanrec; nu->chanrec=NULL;
      while (ch!=NULL) {
	struct chanuserrec *z;
	z=(struct chanuserrec *)nmalloc(sizeof(struct chanuserrec));
	z->next=nu->chanrec; nu->chanrec=z;
	z->flags=ch->flags; z->laston=ch->laston;
...
void get_handle_chaninfo PROTO3(char *,handle,char *,chname,char *,s)
{
  ...
  if (ch==NULL) { s[0]=0; return; }
  if (ch->info==NULL) { s[0]=0; return; }
  strcpy(s,ch->info); return;
}
			     ...
			     unsigned int,add,unsigned int,remove)
{
  ...
  if (chname[0]=='*') {
    chan=chanset; while (chan!=NULL) {
      ...
      if (ch==NULL) add_chanrec(u,chan->name,add,0L);
      else ch->flags=((ch->flags|add)&(~remove)); 
      chan=chan->next;
    }
    return;
  }
  ...
  if (ch==NULL) {
    if (defined_channel(chname)) {
      add_chanrec(u,chname,add,0L); return;
    }
    /* error */
    return;
  }
...
int flags_ok PROTO2(int,req,int,have)
{
  ...
  if ((have&USER_GLOBAL) && (req&USER_PSUEDOOP)) have|=USER_PSUEDOOP;
  if (have&USER_PSUOWN) have|=USER_PSUMST;
  if (have&(USER_PSUMST|USER_PSUOWN)) have|=USER_PSUEDOOP;
  if ((!require_p) && (have&USER_GLOBAL)) have|=USER_PARTY;
  return ((have&req)==req);
}
...
int op_anywhere PROTO1(char *,handle)
{
  ...
  u=get_user_by_handle(userlist,handle);
  if (u==NULL) return 0;
  if (u->flags & USER_GLOBAL) return 1;
  chan=chanset; while (chan!=NULL) {
    ch=get_chanrec(u,chan->name);
    if ((ch!=NULL) && (ch->flags&CHANUSER_OP)) return 1;
    chan=chan->next;
  }
  return 0;
}
...
{ 
  ...
  u=get_user_by_handle(userlist,handle);
  if (u==NULL) return 0;
  if (u->flags & USER_MASTER) return 1;
  chan=chanset; while (chan!=NULL) {
    ch=get_chanrec(u,chan->name);
    if ((ch!=NULL) && (ch->flags&CHANUSER_MASTER)) return 1;
    chan=chan->next;
  }
  return 0;
}
...
{ 
  ...
  u=get_user_by_handle(userlist,handle);
  if (u==NULL) return 0;
  if (u->flags & USER_OWNER) return 1;
  chan=chanset; while (chan!=NULL) {
    ch=get_chanrec(u,chan->name);
    if ((ch!=NULL) && (ch->flags&CHANUSER_OWNER)) return 1;
    chan=chan->next;
  }
  return 0;
}
...
char geticon PROTO1(int,idx)
{
  ...
  if (u==NULL) return '-';
  if (owner_anywhere(dcc[idx].nick)) return '*';
  if (master_anywhere(dcc[idx].nick)) return '+';
  if (get_attr_handle(dcc[idx].nick) & USER_BOTMAST) return '%';
  if (op_anywhere(dcc[idx].nick)) return '@';
  return '-';
}
...
void del_chanrec PROTO2(struct userrec *,u,char *,chname)
{
  ...
  while (ch) {
    if (strcasecmp(chname,ch->channel)==0) {
      if (lst==NULL) u->chanrec=ch->next;
      else lst->next=ch->next;
      if (ch->info!=NULL) nfree(ch->info);
      nfree(ch);
      return;
    }
