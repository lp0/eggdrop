...
void partyaway PROTO3(char *,bot,int,sock,char *,msg)
{
  ...
  for (i=0; i<parties; i++) {
	...
	(party[i].sock==sock)) {
      ...
      party[i].away[60]=0;
      if (!msg[0]) strcpy(party[i].away,"(unknown)");
    }
  }
}
...
void answer_local_whom PROTO2(int,idx,int,chan)
{
  ...
  if (chan==(-1))
    ...
    else dprintf(idx,"Users on channel '%s%s' (%s%d):\n",
  ...
  for (i=0; i<dcc_total; i++) if (dcc[i].type==DCC_CHAT) {
	...
	if (days>0) sprintf(idle," [idle %lud%luh]",days,hrs);
	else if (hrs>0) sprintf(idle," [idle %luh%lum]",hrs,mins);
	else sprintf(idle," [idle %lum]",mins);
      }
      else idle[0]=0;
  ...
  for (i=0; i<parties; i++) {
    if ((chan==(-1)) || ((chan>=0) && (party[i].chan==chan))) {
      c=party[i].flag; if (c=='-') c=' ';
      if (party[i].timer==0L) strcpy(idle," [idle?]");
...
void tell_bottree PROTO1(int,idx)
{
  ...
  s[0]=0; for (i=0; i<tands; i++) if (!tandbot[i].next[0])
  ...
  while (more) {
	...
	if (lev==0) { dprintf(idx,"(( tree error ))\n"); return; }
	ok=0; for (i=0; i<tands; i++) {
	  if (strcasecmp(tandbot[i].next,last[lev-1])==0) {
	    if (strcasecmp(tandbot[i].bot,this)==0) ok=1;
	    else if (ok) {
	      cnt++; if (cnt==1) strcpy(s,tandbot[i].bot);
	    }
	  }
	}
	if (cnt) {
	  ...
	  lev--; if (lev==0) { more=0; cnt=999; }
	  else { more=1; strcpy(this,last[lev]); }
	}
      }
    }
  }
...
int in_chain PROTO1(char *,who)
{
  ...
  if (strcasecmp(who,botnetnick)==0) return 1;
  return 0;
}
...
void cancel_user_xfer PROTO1(int,idx)
{
  ...
  if (dcc[idx].u.bot->status&STAT_SHARE) {
	...
	if ((strcasecmp(dcc[i].host,dcc[idx].nick)==0) &&
      ...
      if (j!=0) {
	killsock(dcc[j].sock);
	dcc[j].sock=dcc[j].type; dcc[j].type=DCC_LOST;
...
int botunlink PROTO3(int,idx,char *,nick,char *,reason)
{
  ...
  for (i=0; i<dcc_total; i++) {
	...
	if (idx>=0) dprintf(idx,"No longer trying to link to %s.\n",
			    dcc[i].nick);
	...
	if (idx>=0) dprintf(idx,"Breaking link with %s.\n",dcc[i].nick);
	tprintf(dcc[i].sock,"bye\n");
	if (reason && reason[0]) {
  ...
  if ((idx>=0) && (nick[0]!='*')) dprintf(idx,"Not connected to that bot.\n");
  if (nick[0]=='*') {
...
int botlink PROTO3(char *,linker,int,idx,char *,nick)
{
  ...
  if (!(get_attr_handle(nick) & USER_BOT)) {
    if (idx>=0) dprintf(idx,"%s is not a known bot.\n",nick);
    return 0;
  }
  ...
  p=strchr(s,':'); if (p==NULL) port=2222;
  else { *p=0; p++; port=atoi(p); }
  if (dcc_total==MAXDCC) {
    if (idx>=0) dprintf(idx,"No more dcc entries left: dcc table is full.\n");
    return 0;
  }
  context;
  correct_handle(nick);
  dcc_total++; i=dcc_total-1;
  dcc[i].port=port; dcc[i].addr=0L;
  strcpy(dcc[i].nick,nick); strcpy(dcc[i].host,s);
  dcc[i].type=DCC_BOT;
  set_tand(i);
  dcc[i].u.bot->status=0; dcc[i].u.bot->timer=time(NULL);
  strcpy(dcc[i].u.bot->linker,linker);
  strcpy(dcc[i].u.bot->version,"(primitive bot)");
  if (idx!=(-1)) putlog(LOG_BOTS,"*","Linking to %s at %s:%d ...",nick,s,port);
  set_fork(i);
  dcc[i].u.fork->x=idx;
...
/*    tandout("*trying %s %s\n",botnetnick,dcc[idx].nick);  */
  ...
  /* try next port */
  killsock(dcc[idx].sock); dcc[idx].sock=getsock(SOCK_STRONGCONN);
  dcc[idx].port++;
...
void cont_link PROTO1(int,idx)
{
  ...
  context;
  ti=dcc[idx].u.fork->u.bot; nfree(dcc[idx].u.fork);
  dcc[idx].u.bot=ti;
  ...
  if (strcasecmp(s,"-")==0) tprintf(dcc[idx].sock,"%s\n",botnetnick);
  else tprintf(dcc[idx].sock,"%s\n%s\n",botnetnick,s);
  context;
  return;
}
...
void tandem_relay PROTO2(int,idx, char *,nick)
{
  ...
  p=strchr(s,':'); if (p==NULL) port=2222;
    ...
    *p++=0; p1=strchr(p,'/');
    if (p1==NULL) port=atoi(p);   /* use link port# */
    else {
      p1++; port=atoi(p1);
    }
  }
  ...
  ci=dcc[idx].u.chat; set_relay(idx); dcc[idx].u.relay->chat=ci;
  dcc[idx].type=DCC_RELAYING;
  set_fork(idx); set_fork(i);
  context;
...
void pre_relay PROTO2(int,idx, char *,buf)
{
  ...
  for (i=0; i<dcc_total; i++)
	...
	(dcc[i].u.fork->u.relay->sock==dcc[idx].sock)) tidx=i;
  if (tidx<0) {
  ...
  if (strcasecmp(buf,"*bye*")==0) {
    ...
    putlog(LOG_MISC,"*","Relay aborted: %s -> %s",dcc[idx].nick,
	   dcc[tidx].nick);
    nfree(dcc[idx].u.fork->u.relay); nfree(dcc[idx].u.fork);
    dcc[idx].u.chat=ci; dcc[idx].type=DCC_CHAT;
    ...
    killsock(dcc[tidx].sock); lostdcc(tidx);
    return;
  }
  context;
}
...
void failed_pre_relay PROTO1(int,idx)
{
  ...
  killsock(dcc[idx].sock); killsock(dcc[tidx].sock);
...
void failed_tandem_relay PROTO1(int,idx)
{
  ...
  for (i=0; i<dcc_total; i++)
	...
	(dcc[i].u.fork->u.relay->sock==dcc[idx].sock)) uidx=i;
  if (uidx<0) {
    putlog(LOG_MISC,"*","Can't find user for relay!  %d -> %d",
  ...
  if (dcc[idx].port>=dcc[idx].u.fork->port+3) {
    ...
    nfree(dcc[uidx].u.fork->u.relay); nfree(dcc[uidx].u.fork);
    dcc[uidx].u.chat=ci; dcc[uidx].type=DCC_CHAT;
    killsock(dcc[idx].sock); lostdcc(idx);
    return;
  }
  killsock(dcc[idx].sock); dcc[idx].sock=getsock(SOCK_STRONGCONN);
...
void cont_tandem_relay PROTO1(int,idx)
{
  ...
  ri=dcc[idx].u.fork->u.relay; nfree(dcc[idx].u.fork);
  dcc[idx].u.relay=ri;
  ri=dcc[uidx].u.fork->u.relay; nfree(dcc[uidx].u.fork);
  dcc[uidx].u.relay=ri;
  dcc[uidx].type=DCC_RELAYING; dcc[idx].type=DCC_RELAY;
...
{
  int i,j; time_t now=time(NULL);
  context;
  for (i=0; i<dcc_total; i++)
    if (dcc[i].type==DCC_BOT) if (dcc[i].u.bot->status&STAT_PINGED) {
  ...
  for (i=0; i<dcc_total; i++)
    if (dcc[i].type==DCC_BOT) if (dcc[i].u.bot->status&STAT_OFFERED)
  ...
  for (i=0; i<dcc_total; i++)
	...
	else dcc[i].u.bot->status&=~STAT_WARNED;
      }
    }
}
