...
/* allow remote unlinks of my sharebots ? */
int share_unlinks = 0;
...
void addbot (char * who, char * from, char * next,char flag, int vernum)
{
   ...
   *ptr = ptr2;
   ...
   if (!rfc_casecmp(next,botnetnick))
...
int partysock (char * bot, char * nick)
{
   int i;
   for (i = 0; i < parties; i++) {
      if ((rfc_casecmp(party[i].bot, bot) == 0) &&
	  (rfc_casecmp(party[i].nick, nick) == 0))
	 return party[i].sock;
   }
   return 0;
}
		     ...
		     char * from, int * idx)
{
   ...
   for (i = 0; i < parties; i++) {
      ...
      if (!rfc_casecmp(party[i].bot, bot) &&
	  (party[i].sock == sock)) {
...
int partynick (char * bot, int sock, char * nick)
{
   ...
   for (i = 0; i < parties; i++) {
      if (!rfc_casecmp(party[i].bot, bot) && (party[i].sock == sock)) {
...
int nextbot (char * who)
{
   ...
   for (j = 0; j < dcc_total; j++)
     if (bot->via && !rfc_casecmp(bot->via->bot, dcc[j].nick)
	  && (dcc[j].type == &DCC_BOT))
       return j;
...
int botunlink (int idx, char * nick, char * reason)
{
   ...
   if (nick[0] == '*')
      dprintf(idx, "%s\n", BOT_UNLINKALL);
   for (i = 0; i < dcc_total; i++) {
      if ((nick[0] == '*') || !rfc_casecmp(dcc[i].nick, nick)) {
	 if (dcc[i].type == &DCC_FORK_BOT) {
	    ...
	    else if ((idx == -3) && (b_status(i) & STAT_SHARE)
		     && !share_unlinks)
	       return -1;
	    dprintf(i, "bye\n");
	    if (reason && reason[0]) {
...
int botlink (char * linker, int idx, char * nick)
{
   ...
   if (!u || !(u->flags & USER_BOT)) {
      if (idx >= 0)
	 dprintf(idx, "%s %s\n", nick, BOT_BOTUNKNOWN);
   } else if (!rfc_casecmp(nick, botnetnick)) {
      if (idx >= 0)
	 dprintf(idx, "%s\n", BOT_CANTLINKMYSELF);
	...
	dprintf(idx, "%s\n", BOT_ALREADYLINKED);
      ...
      for (i = 0; i < dcc_total; i++) 
	if ((dcc[i].user == u) && ((dcc[i].type == &DCC_FORK_BOT)
...
void tandem_relay (int idx, char * nick,int i)
{
   ...
   if (rfc_casecmp(nick, botnetnick) == 0) {
      dprintf(idx, "%s\n", BOT_CANTRELAYMYSELF);
      return;
   }
   /* address to connect to is in 'info' */
