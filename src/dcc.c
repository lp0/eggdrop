...
/* bot timeout value */
int bot_timeout = 60;
/* timeout value for ident lookups */
int identtimeout = 5;
...
static void timeout_dcc_bot_new (int idx) {
   putlog(LOG_BOTS, "*", "Timeout: bot link to %s at %s:%d", dcc[idx].nick,
...
struct dcc_table DCC_CHAT = {
   "CHAT",
   DCT_CHAT|DCT_MASTER|DCT_SHOWWHO|DCT_VALIDIDX|DCT_SIMUL|DCT_CANBOOT|
     DCT_REMOTEWHO,
   eof_dcc_chat,
   dcc_chat,
   0,
...
static void dcc_telnet (int idx, char * buf,int i)
{
   ...
   if (sock < 0) {
      neterror(s);
      putlog(LOG_MISC, "*", "Failed TELNET incoming (%s)", s);
      killsock(sock);
      return;
   }
...
void eof_dcc_identwait(int idx) {
   ...
   putlog(LOG_MISC,"*","Lost connection while identing [%s/%d]",
	  dcc[idx].host,dcc[idx].port);
   for (i = 0;i < dcc_total;i++) 
